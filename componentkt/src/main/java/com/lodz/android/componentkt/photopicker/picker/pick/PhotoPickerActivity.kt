...
import com.lodz.android.componentkt.photopicker.picker.dialog.ImageFolderDialog
import com.lodz.android.componentkt.photopicker.picker.dialog.ImageFolderItemBean
...
internal class PhotoPickerActivity : AbsActivity() {
    ...
    override fun getAbsLayoutId(): Int = R.layout.componentkt_activity_picker
        ...
        mAdapter.setListener(object : PhotoPickerAdapter.Listener {
            override fun onSelected(bean: PickerItemBean, position: Int) {
                ...
                if (mSelectedList.size == pickerBean.maxCount && !bean.isSelected) {// 已经选满图片且用户点击了可选图
                    toastShort(getString(R.string.componentkt_picker_photo_count_tips, pickerBean.maxCount.toString()))
                    return
                }
    ...
    override fun initData() {
        ...
        if (bean.sourceList.isNullOrEmpty()) {// 挑选相册图片
            ...
            mFolderNameTv.setText(R.string.componentkt_picker_custom_photo)
    ...
    private fun getArrowBitmap(@ColorRes color: Int): Bitmap {
        ...
        val path = Path()
        path.moveTo(centerPoint, centerPoint + dp2px(3))
        path.lineTo(centerPoint - dp2px(7), centerPoint - dp2px(3))
        path.moveTo(centerPoint, centerPoint + dp2px(3))
        path.lineTo(centerPoint + dp2px(7), centerPoint - dp2px(3))
        ...
        canvas.drawCircle(centerPoint, centerPoint + dp2px(3), 3f, paint)
        return bitmap
    }
    ...
    private fun takeCameraPhoto() {
        ...
        if (!FileUtils.isFileExists(bean.cameraSavePath) && !FileUtils.createFolder(bean.cameraSavePath)) {// 文件夹不存在且创建文件夹失败
            toastShort(R.string.componentkt_photo_folder_fail)
            return
        }
        ...
        if (!FileUtils.createNewFile(mTempFilePath)) {
            toastShort(R.string.componentkt_photo_temp_file_fail)
            return
        }
        if (Intent(MediaStore.ACTION_IMAGE_CAPTURE).resolveActivity(packageManager) == null) {
            toastShort(R.string.componentkt_no_camera)
            return
        }
