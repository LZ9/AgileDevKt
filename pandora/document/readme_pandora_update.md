...
##### 2025/12/22
1. 升级依赖库，将最小SDK兼容提高到23（6.0）
2. 添加带头布局的RV加载更多适配器
3. 添加RecyclerView判断是否滚动到顶部以及平滑滚动到顶部的方法
4. CltEditView增加输入车牌号类型并进行限制
5. 新增采集专用键盘CollectInputMethodService，支持身份证号、车牌号和通用证件号采集
6. 发布2.1.7版

##### 2025/12/01
1. 九宫格控件增加获取适配器方法
2. CltDynItemsEditView增加输入框最后一条记录启用请求焦点的方法
3. 发布2.1.6版

##### 2025/11/28
1. 采集输入控件CltEditView增加单行展示配置
2. 新增动态条目输入框采集控件CltDynItemsEditView
3. 发布2.1.4版
