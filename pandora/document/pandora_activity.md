...
## 一、AbsActivity
AbsActivity是Pandora里所有Activity封装类的基础，内部实现了Rx的生命周期绑定，EventBus解注册以及Compose的使用。
...
getAbsLayoutId()|获取布局layoutId|如果使用Compose这里可以传0
...
### 4. 使用Compose
 - AbsActivity支持Compose，你只需要加上 **@AbsCompose** 注解 ，然后在RootContentUI()方法里编写的UI，例如：
```
    @Composable
...
```
 - 除了AbsActivity以外的Activity基类**均不支持**Compose
