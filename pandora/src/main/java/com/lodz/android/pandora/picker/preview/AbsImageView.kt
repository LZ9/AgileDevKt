...
abstract class AbsImageView<V : View, T>(private val isScale: Boolean) {
    ...
    /** 点击回调，[view]图片控件，[source]数据，[position]位置 */
    open fun onClickImpl(viewHolder: RecyclerView.ViewHolder, view: V, source: T, position: Int) {}

    /** 长按回调，[view]图片控件，[source]数据，[position]位置 */
    open fun onLongClickImpl(viewHolder: RecyclerView.ViewHolder, view: V, source: T, position: Int):Boolean = false
